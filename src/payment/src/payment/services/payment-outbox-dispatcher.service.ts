...
@Injectable()
export class PaymentOutboxDispatcherService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    this.runtimeHealthService.setComponentStatus('outbox', 'up', {
      message: 'Payment outbox dispatcher started'
    });
    ...
    try {
      ...
      for (const outboxMessage of pendingMessages) {
        try {
          ...
          this.runtimeHealthService.setComponentStatus('outbox', 'up', {
            backlog: Math.max(pendingMessages.length - 1, 0)
          });
        } catch (error) {
      ...
      Logger.error('Payment outbox dispatcher failed', error as Error);
      this.runtimeHealthService.setComponentStatus('outbox', 'degraded', {
